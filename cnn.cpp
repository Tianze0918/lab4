...
//   for (int i0 = 0; i0 < 16; i0++) { // Iterating over set of tiles, each tile size of 16 channels
...
//      */
//     load_output_S0(output, voutput, i0);
//     for (int j = 0; j < 256; j++) {   // Iterating over all input
...
//       //  #pragma HLS unroll factor = 16
//       // for (int i1 = 0; i1 < 16; i1++) {   // Iteratve over 16 output channels within a tile
//         for (int h = 0; h < 16 * 14;
//              h++) { // Note: 16*14 equals 224 (the height dimension)
...
//             // preload the 5×5 patch once
//             float window[5][5];
//             #pragma HLS ARRAY_PARTITION variable=window complete dim=0
//             for (int p = 0; p < 5; p++)
//               for (int q = 0; q < 5; q++)
//                 window[p][q] = input[0][h + p][w + q];
...
// // #pragma HLS ARRAY_PARTITION variable = input cyclic factor = 1 dim = 3
